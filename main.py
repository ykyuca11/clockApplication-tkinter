...
class SaatUygulamasi:
    ...
    def setup_sureolcer(self):
        ...
        self.sureolcer_button = tk.Button(self.sureolcer_frame, text="Süreölçeri Başlat/Durdur",
                                          command=self.sureolcer_baslat_durdur, font=("Helvetica", 24))
    ...
    def setup_zamanlayici(self):
        ...
        self.zamanlayici_button = tk.Button(self.zamanlayici_frame, text="Zamanlayıcı Ayarla",
                                            command=self.zamanlayici_ayarla, font=("Helvetica", 24))
        self.zamanlayici_button.pack(pady=10)
        self.zamanlayici_durdur_button = tk.Button(self.zamanlayici_frame, text="Zamanlayıcıyı Durdur",
                                                   command=self.zamanlayici_durdur, font=("Helvetica", 24))
    ...
    def sureolcer_baslat_durdur(self):
        if not self.sureolcer_durumu:
            self.sureolcer_baslangic_zamani = datetime.now()
            self.sureolcer_durumu = True
            self.sureolcer_guncelle()
        else:
            self.sureolcer_durumu = False

    def sureolcer_guncelle(self):
        if self.sureolcer_durumu:
